...
function ManageTagsModal({
    ...
    return (
        <div role="dialog" aria-label="Manage Tags" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10 }}>
...
function SaveTemplateModal({ draft, onClose, onSave }: {
    ...
    return (
        <div role="dialog" aria-label="Save as Template" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10 }}>
...
function TemplateManagerModal({ templates, onClose, onUse, onDelete }: {
    ...
    return (
        <div role="dialog" aria-label="Templates" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10 }}>
...
export default function HomePage() {
    ...
    async function loadTodos() {
        try {
            ...
            setTodos((current) => {
                const loadedIds = new Set(loadedTodos.map((todo) => todo.id));
                const localTodos = current.filter((todo) => !loadedIds.has(todo.id));
                return sortTodos([...loadedTodos, ...localTodos]);
            });
    ...
    return (
        <main className="app-shell" style={{ maxWidth: 900, padding: 32 }}>
            ...
            <form className="quick-add-card" onSubmit={createTodo} style={{ display: 'grid', gridTemplateColumns: '1fr auto auto auto', gap: 8, margin: '24px 0' }}>
                ...
                <select value={priority} onChange={(event) => setPriority(event.target.value as Priority)} aria-label="Create priority">
            ...
            <div className="list-toolbar">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                    ...
                    <label>Priority <select aria-label="Filter by priority" value={filters.priority} onChange={(event) => setFilters((current) => ({ ...current, priority: event.target.value as Priority | 'all' }))}>
                        <option value="all">All priorities</option>
                        <option value="high">High</option>
                        <option value="medium">Medium</option>
                        <option value="low">Low</option>
                    </select></label>
                    {tags.length > 0 ? <label>Tag <select aria-label="Filter by tag" value={filters.tagId === 'all' ? 'all' : String(filters.tagId)} onChange={(event) => setFilters((current) => ({ ...current, tagId: event.target.value === 'all' ? 'all' : Number(event.target.value) }))}>
                ...
                <ul style={{ padding: 0, listStyle: 'none' }}>
                    {section.items.map((todo) => <li className={`todo-card priority-${todo.priority} ${todo.completed ? 'is-complete' : ''}`} key={todo.id} style={{ borderBottom: '1px solid #ddd', padding: '12px 0' }}>
                        {editingId === todo.id ? <form className="edit-form" onSubmit={saveEdit} style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                            ...
                            {editRecurring ? <select aria-label="Edit recurrence pattern" value={editRecurrencePattern} onChange={(event) => setEditRecurrencePattern(event.target.value as RecurrencePattern)}><option value="daily">Daily</option><option value="weekly">Weekly</option><option value="monthly">Monthly</option><option value="yearly">Yearly</option></select> : null}
                            <select aria-label="Edit reminder" value={editReminderMinutes ?? ''} disabled={!editDueDate} onChange={(event) => setEditReminderMinutes(event.target.value ? Number(event.target.value) as ReminderMinutes : null)}><option value="">No reminder</option><option value="15">15 minutes</option><option value="30">30 minutes</option><option value="60">1 hour</option><option value="120">2 hours</option><option value="1440">1 day</option><option value="2880">2 days</option><option value="10080">1 week</option></select>
